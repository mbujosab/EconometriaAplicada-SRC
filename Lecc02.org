...
$$\Sec{y} = \Sec{t} + \Sec{c} + \Sec{s} + \Sec{e}$$

#+LATEX: \noindent
donde:

- La tendencia "$\Sec{t}$" :: recoge la lenta evolución de la
  media a /largo plazo/.

- El componente estacional "$\Sec{s}$" :: recoge las
  oscilaciones periódicas que se repiten regularmente en ciclos
  estacionales (de año en año, o de semana en semana, etc.).

- El componente cíclico "$\Sec{c}$" :: Cuando aparece
  explícitamente en el modelo, $\Sec{c}$ recoge las
...
@@latex:\noindent@@ donde $\Sec{t}$ es una tendencia lineal no
...
@@latex:\noindent@@ donde $\Sec{t}$ ahora es una /tendencia
cuadrática/ tampoco es un ajuste satisfactorio, pues el componente
irregular $\Sec{e}$ sigue sin parecerse a la realización de un
...
*** Ajuste y componente irregular $\Sec{e}=\Sec{y}-\Sec{t}-\Sec{s}$
   :PROPERTIES:
...
Considere el modelo
$\Sec{y}=\boldsymbol{\mathsf{X}\beta}+\boldsymbol{U}.\;$ Bajo
...
En el modelo
$\Sec{y}=\boldsymbol{\mathsf{X}\beta}+\boldsymbol{U},\;$ si las
...
$$\;\widehat{\boldsymbol{\beta}}=(\boldsymbol{\mathsf{X'}}\boldsymbol{\mathsf{\Sigma}}^{-1}\boldsymbol{\mathsf{X}})^{-1}\boldsymbol{\mathsf{X'}}\boldsymbol{\mathsf{\Sigma}}^{-1}\Sec{y}\;$$
