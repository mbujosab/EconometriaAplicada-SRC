...
- *Paso 1*. Se obtienen los residuos MCO del modelo
  ([[ModeloRegresionLineal]]) con una muestra de tamaño $T$.
  # \ref{eq:ModeloRegresionLineal}
- *Paso 2*. Se realiza un ajuste MCO de los residuos $\hat{E}_t$ sobre
  los regresores del modelo original y sobre los $p$ primeros retardos
  de los residuos.
