...
\end{align*}
no depende de $t$ (por tanto $\boldsymbol{X}$ es estacionario). Es más, por la última ecuación de la lección anterior
$$\;\gamma_k \;=\; \sigma^2\sum\nolimits_{j\in\mathbb{Z}}\psi_{j+k}\psi_j \;=\;
\sigma^2\big(\boldsymbol{\psi}(z)*\boldsymbol{\psi}(z^{-1})\big)_k
\qquad \text{ para } k\in\mathbb{Z}.$$
Y, por tanto
\begin{equation}
...
Multiplicando por $X_{t-k}$, tomando esperanzas y sustituyendo
$X_{t-k}$ por su representación MA($\infty$), donde
...
$\Big(\boldsymbol{\theta}(\mathsf{B})U_t\Big)=\boldsymbol{W}$ y
$\Big(\boldsymbol{\psi}(\mathsf{B})U_t\Big)=\boldsymbol{Y}$, y
recordando ($\ref{eqnLadoIzquierdoYW}$):
\begin{align*}
