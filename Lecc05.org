...
#+BEGIN_EXPORT latex
\noindent
(nótese que esta secuencia sí incluye covarianzas de \underline{todos} los órdenes).
\medskip
...
_serie formal_ (i.e., $cogrado(\boldsymbol{b})\geq{\color{blue}{0}}$)
...
Algunas clases de procesos lineales causales tienen una representación
...
$\;\boldsymbol{\phi}\;$ un _polinomio de grado $p$_ con
...
1) _lineal y estacionario_, exigiremos que $\boldsymbol{\phi}$
...
2) _causal_ exigiremos que las raíces de $\boldsymbol{\phi}$ sean
   mayores que 1 en valor absoluto (_raíces fuera del círculo
   unidad_):
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Tanto $\boldsymbol{\phi}^{-1}$ como $\boldsymbol{\theta}$ son series
