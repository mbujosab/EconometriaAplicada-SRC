...
distribución de $\Sec{X}=(X_t \mid t\in\mathbb{Z})$ usando una
muestra finita (serie temporal) $\Sec{x}=(x_t \mid t=1:n)$.  
...
** Estacionariedad en sentido débil
   ...
   :END:
Un proceso estocástico $\Sec{X}$ se dice *estacionario* (/en
...
- ([[estacionario_media]]) sugiere que las realizaciones de
  $\Sec{X}$ generalmente oscilan entorno a $\mu$.

- ([[estacionario_covarianza]]) sugiere que la variabilidad de las
  realizaciones de $\Sec{X}$ entorno a $\mu$ es constante, pues
...
Cuando $\Sec{X}$ es un proceso estocástico (débilmente) *estacionario* 
...
* Transformaciones de realizaciones de procesos estocásticos NO estacionarios
   ...
   :metadata: (slideshow . ((slide_type . slide)))
   :END:
Un proceso estocástico $\Sec{X}=(X_t \mid t\in\mathbb{Z})$ puede ser
...
$$\ln\Sec{x}=\Big(\ln(x_1),\ldots \ln(x_{114})\Big)$$
...
$$\Sec{y}=\nabla\ln\Sec{x}=\Big(\big[\ln(x_2)-\ln(x_1)\big],\ldots\; \big[\ln(x_{114})-\ln(x_{113})\big]\Big)$$
...
$$\Sec{z}=\nabla_{12}(\nabla\ln\Sec{x})=\nabla_{12}(\Sec{y})=\Big((y_{13}-y_{1}),\ldots\; (y_{113}-y_{101})\Big)$$
...
La tasa logarítmica de variación de $\Sec{y}$ se define como
$z_t=\ln{y_t}-\ln{y_{t-1}};$ es decir

$$\Sec{z}=\nabla\ln\Sec{y} = \Big(\big[\ln(y_2)-\ln(y_1)\big],\ldots\; \big[\ln(y_{n})-\ln(y_{n-1})\big]\Big)$$
...
| Transformación  de la serie temporal @@latex:\newline@@ $\displaystyle \Sec{y}=\{y_t\},\; t=1:n$ | Observaciones                                                                                                                                                             |
|--------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| $\Sec{z}=\ln\Sec{y}=\{\ln y_t\}$                                                                 | A veces independiza la volatilidad del nivel e induce normalidad.                                                                                                         |
| $\Sec{z}=\nabla\Sec{y}=\{y_t-y_{t-1}\}$                                                          | Indica al crecimiento absoluto entre  periodos consecutivos.                                                                                                              |
| $\Sec{z}=\nabla\ln\Sec{y}$                                                                       | Tasa logarítmica de crecimiento. Aproximación del crecimiento relativo entre periodos consecutivos.                                                                       |
| $\Sec{z}=\nabla\nabla\ln\Sec{y}=\nabla^2\ln\Sec{y}$                                              | Cambio en la tasa log, de crecimiento. Indica la “aceleración” en el crecimiento relativo.                                                                                |
| $\Sec{z}=\nabla_{s}\ln\Sec{y}=$ @@latex:\newline\;@@ $\{\ln{y_t}-\ln{y_{t-s}}\}$                 | Tasa de crecimiento acumulada en un ciclo estacional completo ($s$ períodos). Cuando el período estacional es de un año, se conoce como “tasa anual” o “tasa interanual”. |
| $\Sec{z}=\nabla\nabla_{s}\ln\Sec{y}$                                                             | Cambio en la tasa de crecimiento acumulada en un ciclo estacional completo. Es un indicador de aceleración en el crecimiento acumulado.                                   |
# :float sideways 
...
|   | $t$ |     $y_t$ | Incremento en tanto por uno | $\ln y_t$ | Primera dife@@latex:\-@@rencia de $\ln\Sec{y}$ | Incremento en tanto por uno desde $t=1$ | $\ln y_t- \ln y_{1}$ |
|---+-----+-----------+-----------------------------+-----------+-------------------------------------------------------+-----------------------------------------+----------------------|
| # |   1 |      100. |                             |  4.605170 |                                                       |                                         |                      |
