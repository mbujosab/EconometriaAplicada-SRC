...
Pero si $|a|=1$ ninguna de las inversas pertenece a $l_1$
...
$\;a\in\mathbb{R}$, entonces $$\boldsymbol{X}+\boldsymbol{Y}=(X_t+Y_t \mid
t\in\mathbb{Z})$$ y $$a\boldsymbol{X}=\big(a(X_t) \mid t\in\mathbb{Z}\big).$$
...
$$\boldsymbol{X} \quad = \quad \sum_{t=-\infty}^\infty X_t z^t \quad\equiv\quad \boldsymbol{X}(z)$$
...
#+attr_ipynb: (slideshow . ((slide_type . subslide)))
Sea $\boldsymbol{a}$ una secuecia y $\boldsymbol{X}$ un proceso
estocástico tales que $\sum_{k=-\infty}^{\infty}a_kX_{t-k}$ converge
para todo $t$.
...
\end{align*}
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Y en general, si $\sum_{k=-\infty}^{\infty}a_kY_{t-k}$ converge para
todo $t$, entonces
# si \(\fbox{$\boldsymbol{a}\in l_1$}\), entonces
\begin{align*}
\boldsymbol{a}(\mathsf{B})Y_t 
& = (\cdots+a_{-2}\mathsf{B}^{-2}+a_{-1}\mathsf{B}^{-1}+a_0+a_1\mathsf{B}+a_2\mathsf{B}^2+\cdots) Y_t \\
% & = a_0 Y_t + a_1 \mathsf{B}^1 Y_t + a_2 \mathsf{B}^2 Y_t + a_3 \mathsf{B}^3 Y_t \\
